...
BEGIN
  ...
  DELETE FROM public.redemption_tokens rt
    WHERE rt.user_id = uid AND rt.offer_id = _offer_id AND rt.consumed_at IS NULL;
  DELETE FROM public.redemption_tokens rt
    WHERE rt.consumed_at IS NULL AND rt.expires_at <= now() - interval '1 hour';

  exp := now() + interval '3 minutes';
  LOOP
    tries := tries + 1;
    new_token := public.gen_short_code(6);
    EXIT WHEN NOT EXISTS (SELECT 1 FROM public.redemption_tokens rt WHERE rt.token = new_token);
